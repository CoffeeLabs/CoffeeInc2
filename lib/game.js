...
function game(){
  if(counts!=0 && counts%seconds==0){
    ...
    if($.totalStorage.getItem("stat_budget")<=0){
      ...
      updateHeader();
      alert("GAME OVER!\nYou ran out of money and are now bankrupt. But don't worry! It is so easy to get VC these days that you will surely succeed next time. Be sure to try again!");
      newGame();
    }
...
function investorDeny(type,pic,name,skill,salary){
  ...
  if(type==1){// employee hiring
    ...
    for(var i=0; i<investorInformation.length;i++){
      alert(investorInformation[i][5]);
      if((Math.random()*investorInformation[i][5])>0.95){
        deny=true;
        break;
      }
    }
...
function about() {
  alert('We are CoffeeLabs. We are badass.\n\nThis simple, hacky game has been built by:\n\nJulian Gorfer\nTobias Bernard\nGianluca Stivan\nStefano Mich\nJulian Gruber\n\nfor Pekka Abrahamsson\'s Management Engineering class in 2012/2013.\n\nAll code and artwork is released under the GNU GPLv3 or later and available on Github.');
}
