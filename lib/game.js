...
function game(){
  if(counts!=0 && counts%seconds==0){
    ...
    /*random effects here please GO*/
    if(counts/seconds>1 && employeeInformation.length>1){
      var randomEffect=Math.round(Math.random()*10);
      switch(randomEffect){
        case 0:
        case 10:
          alert("You are getting a tax refund of "+Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.05))+"$");
          $.totalStorage.setItem("stat_budget",$.totalStorage.getItem("stat_budget")+Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.05)));
          break;
        case 4:
          alert("Taxes are due! You have to pay "+Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.7))+"$");
          $.totalStorage.setItem("stat_budget",$.totalStorage.getItem("stat_budget")-Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.7)));
          break;
        case 6:
          alert("You got a parking ticket and have to pay "+Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.2))+"$");
          $.totalStorage.setItem("stat_budget",$.totalStorage.getItem("stat_budget")-Math.round($.totalStorage.getItem("stat_budget")*(Math.random()*0.7)));
          break;
        default:
          break;
...
function investorDeny(type,pic,name,value1,value2){
  ...
  if(type==1){// employee hiring
    ...
    for(var i=0; i<length;i++){
      if((Math.random()+investorInformation[i][5]/10-0.5)>0.95){
    ...
    if(deny==false)
      hireEmployee(pic, name, value1, value2);
    else
      generateNewEmp();
  }else{
    deny=false;
    ...
    if(deny==false)
      hireInvestor(pic, name, value1, value2);
    else
      generateNewInv();
  }
}
