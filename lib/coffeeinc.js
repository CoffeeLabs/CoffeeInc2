...
function decSalary(){
  if((empSalary-31)>=0)
    ...
    empSalary=0;
  decSat=Math.round((Math.random()+0.8)/100);
  if((empSat-parseInt(decSat))<0)
    empSat=0;
  else
    empSat-=parseInt(decSat);
...
function incSalary(){
  if((empSalary+31)<=2500)
    empSalary+=31;
  else
    empSalary=2500;
  incSat=Math.round((Math.random()+0.8)/10);
  if((empSat+parseInt(incSat))>100)
    empSat=100;
  else
    empSat+=parseInt(incSat);
  employeeInformation[position][6]=empSalary;
  employeeInformation[position][4]=empSat;
  $.totalStorage.setItem("emp", employeeInformation);
  document.getElementById('empProfileSat').innerHTML="Satisfaction: "+empSat;
...
function hireEmployee(empPic,empName,empSkill,empSat,empSalary){
  ...
  for(var i=0;i<employeeInformation.length;i++){
    employeeInformation[i][2]+=satInc;
  }
  employeeInformation.push([empPic,empName,empSkill,5,empSat,empSalary,empSalary]);
  $.totalStorage.setItem("avSalary", $.totalStorage.getItem("avSalary")+empSalary);
